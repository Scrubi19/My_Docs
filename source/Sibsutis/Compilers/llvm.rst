...
Оптимизация Tail Recursion Elimination (TRE)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code-block:: c
	...
	// рекурсивный поиск суммы от 1 до n
	int recSum(int n) {
		if (n == 0)
			return 0;
	return n + recSum(n - 1);
...
Проход оптимизации: Анализ
"""""""""""""""""""""""""""""

 * **static bool canTRE(Function &F)** - сканирует указанную функцию на предмет инструкций alloca. Если он содержит какие-либо динамические аллоки, возвращает false
 ...
 * **void callUsesLocalStack(CallBase &CB, bool IsNocapture)** — функция добавляет значения в два списка:
  ...
  * *AllocaUsers* - вызовы alloc пользователя

  * *EscapePoints* - такие call, которые могут записывать что-то в память на стеке, исключение их из CallBase может привезти к утечке значения
...
Проход оптимизации: Преобразование
"""""""""""""""""""""""""""""""""""""

TailRecursionEliminator - класс отвечает за смену всех хвостовых рекурсивных вызовов на циклы и включает след.функции:

 * **CallInst findTRECandidate( BasicBlock BB, ..)** - поиск в базовом блоке вызов call, который будет подходить для оптимизации

 * **bool processBlock(BasicBlock &BB, bool CannotTailCallElimCallsMarkedTail)** — создает базовый блок, который является телом нового цикла, который получился в результате устранения хвостовой рекурсии

 * **void insertAccumulator(Instruction AccRecInstr)** — инициализация и вставка переменной аккумулятора, в которой будет храниться результат функции

 * bool **eliminateCall(CallInst CI)** — удаление всех хвостовых вызовов из IR представления


Пример работы оптимизации (Успех)
""""""""""""""""""""""""""""""""""""
...
`ссылка на презентацию <TRE.pdf>`_
